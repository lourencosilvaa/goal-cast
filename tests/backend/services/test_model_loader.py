...
class TestModelLoader:
    ...
    def test_download_calls_snapshot_download(self) -> None:
        ...
        with patch("src.backend.services.model_loader.snapshot_download") as mock_dl:
    ...
    def test_download_raises_on_failure(self) -> None:
        loader = self._make_loader()
        with patch(
            "src.backend.services.model_loader.snapshot_download",
    ...
    def test_get_model_path_returns_joblib(self) -> None:
        loader = self._make_loader()
        with patch("src.backend.services.model_loader.snapshot_download", return_value="/tmp/test_hf"):
            loader.download()
